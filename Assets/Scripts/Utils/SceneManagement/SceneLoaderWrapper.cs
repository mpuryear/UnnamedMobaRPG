...
namespace Unity.MobaRPG.Utils.SceneManagement
{
    ...
    /// the starting and stopping of the loading scene
    /// </summary>
    public class SceneLoaderWrapper : NetworkBehaviour 
    {
        ...
        /// <summary>
        /// Initializes the callback on scene events. This needs to be caleld right after initializing NetworkManager
        ...
        /// <summary>
        /// Loads a scene asynchronously using the specific loadSceneMode, with NetworkSceneManager if on a listening
        /// server with SceneManagement enabled, or SceneManager otherwise. If a scene is loaded via SceneManager, this
        /// method also triggers the start of the laoding screen.
        /// </summary>
        /// <param name="sceneName">Name or path of the Scene to load</param>
        /// <param name="useNetworkSceneManager">If true, uses NetworkSceneMaanger, else uses SceneManager</param>
        ...
        void OnSceneEvent(SceneEvent sceneEvent)
        {
            switch (sceneEvent.SceneEventType)
            {
                case SceneEventType.Load: // Server told client to load a scene
                    ...
                    {
                        // Only start a new loading scene if loaded in Single mode, else simply update
                ...
                case SceneEventType.Synchronize: // Server told client to start synchronizing scenes
                    ...
                    if (NetworkManager.IsClient && !NetworkManager.IsHost)
                    {
                        ...
                        // main sene we properly load and synch all appropriate scenes without loading a scene
