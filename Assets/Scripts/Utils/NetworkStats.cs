...
namespace Unity.MobaRPG.Utils
{
    ...
    /// <summary>
    /// This utility helps showing networks statistics at runtime.
    /// This Component attaches to any networked object
    /// It will spawn all of the needed text and canvas.
    /// 
    ...
    {
        // For a value like RTT an exponential moving average is better indication of the current rtt and fluctuates less.
        struct ExponentialMoveingAverageCalculator
        {
            readonly float m_Alpha;
            float m_Average;
            public float Average => m_Average;

            public ExponentialMoveingAverageCalculator(float average)
            {
        ...
        ExponentialMoveingAverageCalculator m_MobaRPGRTT = new ExponentialMoveingAverageCalculator(0);
        ExponentialMoveingAverageCalculator m_UtpRTT = new ExponentialMoveingAverageCalculator(0);
        ...
        // When receiving pong client RPCs, we need to know when the initiating ping sent to it so we can calculate its individual RTT
        ...
        void FixedUpdate()
        {
            if (!m_IsServer)
            {
                ...
                if (m_TextStat != null)
                {
                    m_TextToDisplay = $"RTT: {(m_MobaRPGRTT.Average * 1000).ToString("0")} ms;\nUTP RTT {m_UtpRTT.Average.ToString("0")} ms";
        ...
        void PongClientRPC(int pingId, ClientRpcParams clientParams = default)
        {
            ...
            m_MobaRPGRTT.NextValue(Time.realtimeSinceStartup - startTime);
        }
