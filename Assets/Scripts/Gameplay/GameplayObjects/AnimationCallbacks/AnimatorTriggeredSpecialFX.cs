...
namespace Unity.MobaRPG.Gameplay.GameplayObjects.AnimationCallbacks
{
    /// <summary>
    ...
    public class AnimatorTriggeredSpecialFX : MonoBehaviour
    {
        ...
        private IEnumerator CoroPlayStateEnterFX(AnimatorNodeEntryEvent eventInfo)
        {
            ...
            var instantiateFX = Instantiate(eventInfo.m_Prefab, parent);
            instantiateFX.transform.localPosition += eventInfo.m_PrefabParentOffset;
            ...
            {
                instantiateFX.transform.SetParent(null);
            }
            ...
            if (eventInfo.m_PrefabCanBeAbortedUntilSecs > 0)
            {
                ...
                while (timeRemaining > 0 && instantiateFX)
                {
        ...
        private AudioSource GetAudioSourceForLooping()
        {
            ...
            Debug.LogWarning($"{name} doesn't have enough AudioSources to loop all desired sound effects. (Have {m_AudioSources.Length}, new atleast 1 more)", gameObject);
            return null;
        }
    ...
    /// animator node names we reference are actually sued by our Animator. We
    ...
    public class AnimatorTriggeredSpecialFXEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // It's really hard to follow the inspector when there's a lot of these compononets on the same GameObject... so let's add a bit of whitespace
        ...
        private void ValidateNodeNames(AnimatorTriggeredSpecialFX fx)
        {
            ...
            {
                for (int j = 0; j < fx.m_EventsOnNodeEntry.Length; j++)
                {
