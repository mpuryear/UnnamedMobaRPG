...
using Unity.MobaRPG.Gameplay.GameplayObjects.Character.AI;
...
namespace Unity.MobaRPG.Gameplay.GameplayObjects.Character
{
    ...
    public class ServerCharacter : NetworkBehaviour, ITargetable
    {
        ...
        void InitializeHitPoints()
        {
            ...
            Debug.Log($"Initialized HP to {HitPoints}");
        }
        ...
        {
            // the character needs to be a alive in order to be able to play actions
            if (LifeState == LifeState.Alive && !m_Movement.IsPerformingForcedMovement())
            {
        ...
        void ReceiveHP(ServerCharacter inflicter, int HP)
        {
            ...
            {
#if UNITY_EDITOR || DEVELOPMENT_BUID
                // Dont apply damage if god mode is on
            ...
            if (m_AIBrain != null)
            {
                // let the brain know about the modified amount of damage received
                m_AIBrain.ReceiveHP(inflicter, HP);
            }

            // We can't currently heal a dead character back to Alive state.
            // Thats handled by a separate function.
            if (HitPoints <= 0)
            {
                if (IsNpc)
                {
