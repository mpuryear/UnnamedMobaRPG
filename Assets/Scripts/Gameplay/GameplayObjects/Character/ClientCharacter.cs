...
using Unity.MobaRPG.CameraUtils;
...
namespace Unity.MobaRPG.Gameplay.GameplayObjects.Character
{
    ...
    public class ClientCharacter : NetworkBehaviour
    {
        ...
        /// <summary>
        /// This PRC is invoked on the client when the active action FXs need to be canceleled (e.g. when the character has been stunned)
        /// </summary>
        [ClientRpc]
        ...
        public override void OnNetworkSpawn()
        {
            ...
            // sync our visualization position & rotation to the most up to date version received from our server
            ...
            if (!m_ServerCharacter.IsNpc)
            {
                ...
                // ...and visualize the curent char-select value that we know about
                SetAppearanceSwap();
                ...
                if (m_ServerCharacter.IsOwner)
                {
                    ...
                    {
                        // TODO: revisit; anticipated actions would play twice on he host
                        if (!IsServer)
                        {
        ...
        {
            // if you are trying to figure out who calls this method, it's "magic". The Unity Animation Event system takes method names as string,
