...
namespace Unity.MobaRPG.Gameplay.GameplayObjects
{
    ...
    public class ServerWaveSpawner : NetworkBehaviour
    {
        ...
        /// <summary>
        /// Coroutine for spawning prefabs clonse in waves, waiting a duration before spawning a new wave.
        ...
        /// <summary>
        /// Coroutine that spawns a wave of prefab clones, with some time between spwans.
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Returns the current max number of entities we should try to maintaint.
        ...
        bool IsAnyPlayerNearbyAndVisible()
        {
            ...
            foreach (var serverCharacer in PlayerServerCharacter.GetPlayerServerCharacters())
            {
                if (!m_DetectStealthyPlayers && serverCharacer.IsStealthy.Value)
                {
                    // we don't detect stealthy players
                    continue;
                }

                var playerPosition = serverCharacer.physicsWrapper.Transform.position;
                ...
                // player is in range, so check LoS
