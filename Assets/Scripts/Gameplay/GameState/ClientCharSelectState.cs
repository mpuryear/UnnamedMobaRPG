...
using Unity.MobaRPG.Gameplay.UI;
using TMPro;
...
namespace Unity.MobaRPG.Gameplay.GameState
{
    ...
    public class ClientCharSelectState : GameStateBehaviour
    {
        ...
        void OnLobbyPlayerStateChanged(NetworkListEvent<NetworkCharSelection.LobbyPlayerState> changeEvent)
        {
            ...
            // now lets find our local player in the list and update the character/info box appropriately
            ...
            if (localPlayerIdx == -1)
            {
                // We aren't current participating in the lobby!
        ...
        /// <summary>
        /// Ineternal utility that sets the character-graphics and class-info box based on
        ...
        void UpdateSeats()
        {
            ...
            // now actually update the seats in UI
            for (int i = 0; i < m_PlayerSeats.Count; i++)
            {
        ...
        /// <summary>
        /// Turns on the UI elements for a specific "lobby mode", and turns off UI elements for all other modes.
