...
namespace Unity.MobaRPG.Gameplay.GameState
{
    ...
    public class ServerCharSelectState : GameStateBehaviour
    {
        ...
        /// if so, we lock in the whole lobby, save state, adn begin the transition to gameplay
        /// </summary>
        void CloseLobbyIfReady()
        {
            foreach (NetworkCharSelection.LobbyPlayerState playerInfo in networkCharSelection.LobbyPlayers)
            {
                ...
                {
                    return; // nope, atleast one player isn't lock in yet!
                }
            }

            // everybody's ready at the same tiem! Lock it down!
        ...
        /// <summary>
        /// Cancels the process closing the lobby, so that if a new player joins, they are able to choose a character.
        /// </summary>
        ...
        void SaveLobbyResults()
        {
            foreach (NetworkCharSelection.LobbyPlayerState playerInfo in networkCharSelection.LobbyPlayers)
            {
                ...
                if (playerNetworkObject && playerNetworkObject.TryGetComponent(out PersistentPlayer persistentPlayer))
                {
                    ...
                    // it'd be great to simplify this with something like a NetworkScriptableObject :(
        ...
        IEnumerator WaitToEndLobby()
        {
            ...
            SceneLoaderWrapper.Instance.LoadScene("BattleArena", useNetworkSceneManager: true);
        }
        ...
        void OnSceneEvent(SceneEvent sceneEvent)
        {
            ...
            // When the client finished loading the Lobby Map, we will need to Seat it
