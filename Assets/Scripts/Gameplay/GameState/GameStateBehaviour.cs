...
namespace Unity.MobaRPG.Gameplay.GameState
{
    ...
    /// <summary>
    /// A pecial component that represents a discrete game state and its dependencies. The special feature it offeres is 
    /// that is provides some guarantees that only one such GameState will be running at a time.
    /// </summary>
    /// <remarks>
    /// Q: what is the relationship between a GameState and a Scane?
    /// A: There is a 1-to-many relationship between stats and scenes. That is, every scene corresponds to exactly one state,
    ...
    public abstract class GameStateBehaviour : LifetimeScope
    {
        ...
        /// <summary>
        /// This is a single active GameState object. There can be only one.
        /// </summary>
        ...
        protected virtual void Start()
        {
            if (s_ActiveStateGO != null)
            {
                ...
                // On the host, this might return either the client or the server version, but it doesn't matter which;
                // we are only curious about its type and persist state.
                ...
                // othewrwise, the old state is going away. Either it wasn't a Persisting state, or it was,
