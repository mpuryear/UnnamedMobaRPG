...
namespace Unity.MobaRPG.Gameplay.UserInput
{
    /// <summary>
    ...
    public class ClientInputSender : NetworkBehaviour 
    {
        ...
        // The movement input rate is capped at 40ms (or 25fps). This provides a nice balance between responsiveness and
        ...
        /// in other matters will sue the stateful target stored in NetworkCharacterState.
        /// </summary>
        ...
        bool IsRealeaseStyle(SkillTriggerStyle style)
        {
        ...
        /// <summary>
        /// Number of ActionRequests that have been queued since the last FixedUpdated/
        /// </summary>
        ...
        public override void OnNetworkSpawn()
        {
            ...
            {
                enabled = false;
                // don't need to do anything else if not the owner
                return;
            }
        ...
        void FixedUpdate()
        {
            ...
            for (int i = 0; i < m_ActionRequestCount; i++)
            {
                if (m_CurrentSkillInput != null)
                {
                    ...
                    if (IsRealeaseStyle(m_ActionRequests[i].TriggerStyle))
                    {
                        m_CurrentSkillInput.OnReleaseKey();
                    }
                }
                else if (!IsRealeaseStyle(m_ActionRequests[i].TriggerStyle))
                {
            ...
            if (m_MoveRequest)
            {
                ...
                if ((Time.time - m_LastSentMove) > k_MoveSendRateSeconds)
                {
                    ...
                    var groundHits = Physics.RaycastNonAlloc(
                        ray,
                        k_CachedHit,
        ...
        /// <summary>
        /// Perform a skill in response to some input trigger. This is the common method to which all input-drive skill plays funnel.
        /// </summary>
        ...
        void PopulateSkillRequest(Vector3 hitPoint, ActionID actionID, ref ActionRequestData resultData)
        {
            ...
            // most skill types should implicitly close distance. THe ones that don't are explicitly set to fasle in the following switch.
            ...
            switch (ActionConfig.Logic)
            {
                ...
                case ActionLogic.LaunchProjectile:
                    ...
                    return;
                case ActionLogic.ChargedLaunchProjectile:
        ...
        void UpdateAction1()
        {
                ...
                )
            {
                ...
                && !charState.IsNpc)
            {
                // special case: when we hafve a player selected, we change the meaning of the basic action
