...
namespace Unity.MobaRPG.Gameplay.UI
{
    ...
    public class ConnectionStatusMessageUIManager : MonoBehaviour
    {
        ...
        [Inject]
        void InjectDependencies(ISubscriber<ConnectStatus> connectStatusSub, ISubscriber<ReconnectMessage> reconnectMessage)
        {
            m_Subscriptions = new DisposableGroup();
            m_Subscriptions.Add(connectStatusSub.Subscribe(OnConnectStatus));
            m_Subscriptions.Add(reconnectMessage.Subscribe(OnReconnectMessage));
        }
