...
namespace Unity.MobaRPG.Gameplay.UI
{
    public class LobbyCreationUI : MonoBehaviour
    {
        ...
        {
            var sanitized = LobbyUIMediator.Sanitize(m_LobbyNameInputField.text);
            Debug.Log($"LobbyCreationUI OnCreateClick sanitized: {sanitized}");
            m_LobbyUIMediator.CreateLobbyRequest(sanitized, m_IsPrivate.isOn);
        }
