...
namespace Unity.MobaRPG.Gameplay.Actions
{
    ...
    public class ServerActionPlayer
    {
        ...
        /// the time-length of the queue, since actions are allowed to block indefinitely. But this is still a useful estimate the prevents
        /// us from piling up a large number of small actions.
        /// </summary>
        private const float k_MaxQueueTimeDepth = 0.2f;

        private ActionRequestData m_PendingSynthesizedAction = new ActionRequestData();
        private bool m_HasPendingSynthesizedAction;

        // Most recent action played used to broadcast outcomes to all users chat boxes
        [SerializeField]
        NetworkVariable<Action> m_MostRecentAction = new NetworkVariable<Action>(null);
        public NetworkVariable<Action> MostRecentAction => m_MostRecentAction;
        ...
        /// used too recently. (Meaning that its ReuseTimeSeconds hasn't elapsed since that last use.)
        /// </summary>
        ...
        private void StartAction()
        {
            if (m_Queue.Count > 0)
            {
                ...
                m_MostRecentAction.Value = m_Queue[0];
        ...
        /// </summary>
        /// <returns>The new index aof the Action being operated on.</returns>
        ...
        private int SynthesizeTargetIfNecessary(int baseIndex)
        {
                ...
                && targets[0] != m_ServerCharacter.TargetId.Value)
            {
                ...
                // This shouldn't run redundantly, because the next time the base Action comes to play, its Target
        ...
        public void OnUpdate()
        {
            ...
            if (m_Queue.Count > 0 && m_Queue[0].ShouldBecomeNonBlocking())
            {
                ...
                // the player can enqueu other actions in the meantime.)
                m_NonBlockingActions.Add(m_Queue[0]);
                AdvanceQueue(false);
            }
        ...
        /// </summary>
        /// <returns>The total "time depth" of the queue, or how long it would take to play in seconds, i no more actions were added.</returns>
        ...
        /// </summary>
        /// <param name="activityThatOccured">The type of event that has occurred</param>
        public virtual void OnGameplayActivity(Action.GameplayActivity activityThatOccured)
        {
            if (m_Queue.Count > 0)
            {
                m_Queue[0].OnGameplayActivity(m_ServerCharacter, activityThatOccured);
            }
            foreach (var action in m_NonBlockingActions)
            {
                action.OnGameplayActivity(m_ServerCharacter, activityThatOccured);
            }
        }


        /// <summary>
        /// Cancels the first instance of the given ActionLogic that is currently running, or all isntances if cancelAll is set to true.
        ...
        /// <param name="cancellAll">If true will cancel all instance; if false will cancel the first running instance.</param>
        /// <param name="exceptThis">If set, will skip this action (useful for actions canceling toher instances of themselves).</param>
        public void CancelRunningActionsByLogic(ActionLogic logic, bool cancellAll, Action exceptThis = null)
        {
            for (int i = m_NonBlockingActions.Count - 1; i >= 0; --i)
            {
