...
namespace Unity.MobaRPG.Gameplay.Actions
{
    ...
    ///      1. Do the DetectFor operations once--in Start.
    ///         Pros: Simple! Only one physics cast per swing--saves on perf.
    ///         Cons: Is unfair. You can step out of the swing of an attack, but no mater how far you go, you'll still be hit. The reverse is also true--you can
    ///               "step into an attack", and it wont affect you. This will feel terrible to the attacker.
    ///      2. Do the DetectFoe operation once--in Update. Send a separate RPC to the targeted entity tilling it to play its hit react.
    ///         Pros: Always shows the correct behavior. The entitty that gets hit plays its hit react (if any).
    ...
    /// </remarks>
    [CreateAssetMenu(menuName = "MobaRPG/Actions/Melee Action")]
    public partial class MeleeAction : Action
    {
        ...
        /// enemy.
        /// </summary>
        /// <param name="isNPC">true if the attack is an NPC (and therefore should hit PCs). False for the reverse</param>
        ...
        public static IDamageable GetIdealMeleeFoe(bool isNPC, Collider ourCollider, float meleeRange, ulong preferredTargetNetworkId)
        {
            ...
            // everything that got hit by the raycast should have an IDamageable component, so we can retreive that and see if they're appropriate targets.
