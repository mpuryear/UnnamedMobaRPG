...
namespace Unity.MobaRPG.Gameplay.Actions
{
    ...
    /// </summary>
    [CreateAssetMenu(menuName = "MobaRPG/Actions/Launch Projectile Action")]
    public class LaunchProjectileAction : Action
    {
        ...
        protected void LaunchProjectile(ServerCharacter parent)
        {
            if (!m_Launched)
            {
                ...
                // point the projectile where it was clicked if possible
                if (Data.Direction != Vector3.zero) 
                {
                    no.transform.forward = Data.Direction;
                } 
                else
                {
