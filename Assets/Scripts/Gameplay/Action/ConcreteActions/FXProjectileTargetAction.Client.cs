...
namespace Unity.MobaRPG.Gameplay.Actions
{
    public partial class FXProjectileTargetedAction
    {
        ...
        public override bool OnUpdateClient(ClientCharacter clientCharacter)
        {
            ...
            // we keep going until the projectiles duration ends
        ...
        public override void CancelClient(ClientCharacter clientCharacter)
        {
            if (m_Projectile)
            {
                // we aborted post-projectile-launch (somehow)! Tell the graphics! (It will destroy itself, possibly after playing somore more FX)
                m_Projectile.Cancel();
            }
        }
        ...
        {
            var projectileGO = Instantiate(Config.Projectiles[0].ProjectilePrefab, parent.transform.position, parent.transform.rotation, null);

            var projectile = projectileGO.GetComponent<FXProjectile>();
            if (!projectile)
            {
                throw new System.Exception($"FXProjectileTargetedAction tried to spawn projectile {projectileGO.name}, as directed for action {name}, but the object doesn't have a FXProjectile component!");
            }
        ...
        public override void AnticipateActionClient(ClientCharacter clientCharacter)
        {
            ...
            // See if this is going to be a "miss" because the player tried to click throguh a wall. If so,
