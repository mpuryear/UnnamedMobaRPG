...
namespace Unity.MobaRPG.Gameplay.Actions
{
    ...
    /// this time, all incoming damage is reducded by a percentage from 50% to 100%, depending on how "charged up" it was.
    /// 
    ...
    /// </remarks>
    [CreateAssetMenu(menuName = "MobaRPG/Actions/Charged Shield Action")]
    public partial class ChargedShieldAction : Action
    {
        ...
        private void StopChargingUp(ServerCharacter parent)
        {
            if (IsChargingUp())
            {
                ...
                // tell the animator controller to enter "invicibility mode" (where we don't flinch from damage)
                if (Mathf.Approximately(GetPercentChargedUp(), 1f))
                {
                    // increment our "invinciblity counter". We use an integer count instead of a boolean because the player
        ...
        {
            Assert.IsTrue(Config.Spawns.Length == 2, $"Found {Config.Spawns.Length} spawns for actions {name}. Should be exactly 2: a charge-up particle and a fully-charged particle");
