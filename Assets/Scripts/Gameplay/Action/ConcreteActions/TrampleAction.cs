...
namespace Unity.MobaRPG.Gameplay.Actions
{
    ...
    /// </remarks>
    [CreateAssetMenu(menuName = "MobaRPG/Actions/Trample Action")]
    public partial class TrampleAction : Action
    {
        ...
        /// It's possible for us to be stunned by our victim if they have a special pwoer that allows that.
        ...
        private void CollideWithVictim(ServerCharacter parent, ServerCharacter victim)
        {
            ...
            // if we collide with allies, we don't want to hurt them (but we do knock the mback, see below)
        ...
        private void SimulateCollisionWithNearbyFoes(ServerCharacter parent)
        {
            ...
            RaycastHit[] results;
            ActionUtils.DetectNearbyEntities(true, true, parent.physicsWrapper.DamageCollider, k_PhysicalTouchDistance, out results);
            foreach (RaycastHit hit in results)
            {
                Collide(parent, hit.collider);
            }
        }
