...
namespace Unity.MobaRPG.Gameplay.Actions
{
    ...
    /// is confirmed and the mouse is clicked, it'll send the appropriate RPC to the server, triggering the AoE server side gameplay logic.
    /// The server side gameplay action will then trigger the client side resulting FX.
    /// This action's flow is this: (Cient) AoEActionInput --> (Server) AoEAction --> (Client) AoEActionFX
    /// </summary>
    public class AoeActionInput : BaseActionInput
    {
        ...
        /// have to drag her mouse from the button to the firing location). Tracking a mouse-down mouse-up cycle means thats a user can
        ...
        void Update()
        {
            if (PlaneRaycast(k_Plane, m_Camera.ScreenPointToRay(Input.mousePosition), out Vector3 pointOnPlane) &&
                NavMesh.SamplePosition(pointOnPlane, out m_NavMeshHit, 10f, NavMesh.AllAreas))
            {
        ...
        /// </summary>
        /// <remarks>
        /// Based on documented examples here: https://docs.unity3d.com/ScriptReference/Plane.Raycast.html
        /// </remarks>
