...
using Unity.MobaRPG.VisualEffects;
using Unity.Netcode;
using UnityEngine;
using BlockingMode = Unity.MobaRPG.Gameplay.Actions.BlockingModeType;
...
namespace Unity.MobaRPG.Gameplay.Actions
{
    ...
    /// There is only ever one active Action (also called the "blocking" action) at a time on a characer, but multiple
    ...
    /// A better action system would need to be more accessible and customizable by game designers and allow more design emergence. It'd have ways to define smaller atomic action steps and have a generic way to define and access character data. 
    /// It would also need to be more performant, as actions would scale with your number of characters and concurrent actions. 
    /// </remarks>
    public abstract class Action : ScriptableObject
    {
        ...
        /// </summary>
        /// <returns> true to keep running, false to stop. The Action will stop be default when its duration expires, if its has a duration set. </returns>
        ...
        /// called again. If the Action wants to immeidately segue into a different Action, it can do so here. The new
        ...
        {
            PercentHealingReceived, // unbuffed value is 1.0. Reducing to 0 wouild mean "no healing". 2 would mean "double healing"
        ...
        /// </summary>
        /// <remarks>
        /// When a GameplayActivity pf AttackedByEnemy or Healed happens, OnGameplayAction() is called BEFORE BuffValue() is called.
        /// </remarks>
        /// <param name="serverCharacter"></param>
        ...
        /// </remarks>
        /// <returns>true to play, false to be immediately cleanred up.</returns>
        ...
        /// <summary>
        /// End if always called when the ActionFX finishes playing. This is a good place for a derived classes to put
        /// wrap-up logic (perhaps playing the "puff of smoke" that rises when a persistent fire AOE goes away). Derived
        /// classes aren't required to call base.End(); by default the method just calls 'Cancel', to handlke the 
        ...
        /// <summary>
        /// Cancel is called when ActionFX is interrupted prematurely. It is kept logically distinct from End to allow
        ...
        public static bool ShouldClientAnticipate(ClientCharacter clientCharacter, ref ActionRequestData data)
        {
            ...
            // at present all Actions anticipate except for the Target action, which runs a single instance on the client and is
        ...
        /// <summary>
        /// Utility function that instiates one of the graphics from the Spawns list.
        /// If parentToOrigin is true, the new graphics are parented to the origin Transform.
        /// If false, they are positioned/oriented the same way but are not parented.
        /// </summary>
        protected SpecialFXGraphic InstantiateSpecialFXGraphic(GameObject prefab, Transform origin, bool parentToOrigin)
        {
            ...
            {
                throw new System.Exception($"One of the Spawns on action {this.name} does not have a SpecialFXGraphic component and can't be instantied!");
            }
