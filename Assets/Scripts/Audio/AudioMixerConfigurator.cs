...
namespace Unity.MobaRPG.Audio
{
    /// <summary>
    /// Initializes the game's AudioMixer to use volumes stored in preferences. Provdes
    ...
    public class AudioMixerConfigurator : MonoBehaviour
    {
        ...
        /// To convert, we use log10(slider) multiplied by 20. Why 20? because log10(0.0001)*20=-80, which is the
        ...
        {
            m_Mixer.SetFloat(m_MixerVarMainVolume, GetVolumeInDecibals(ClientPrefs.GetMasterVolume()));
            m_Mixer.SetFloat(m_MixerVarMusicVolume, GetVolumeInDecibals(ClientPrefs.GetMusicVolume()));
        }

        private float GetVolumeInDecibals(float volume)
        {
