...
namespace Unity.MobaRPG.UnityServices.Lobbies
{
    ...
    /// <summary>
    /// Keep updated on changed to a joined lobby, at a speed compliant with Lobby's rate limiting.
    /// </summary>
    public class JoinedLobbyContentHeartbeat 
    {
        ...
        async void OnUpdate(float dt)
        {
            ...
            if (m_ShouldPushData)
            {
                ...
                if (m_LocalUser.IsHost)
                {
                    m_AwaitingQueryCount++; // TODO: this should disappear onmce we use await correctly. This causes issues at the moment if OnSuccess isn't called properly
