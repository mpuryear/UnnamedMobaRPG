...
namespace Unity.MobaRPG.UnityServices.Lobbies
{
    ...
    public class LobbyServiceFacade : IDisposable, IStartable
    {
        ...
        const float k_HearbeatPeriod = 8; // The heartbeat must be rate-limited to 5 calls per 30s. We'll aim for longer in case periods don't align.
        ...
        async void UpdateLobby(float unused)
        {
            ...
            try
            {
                ...
                if (!m_LocalUser.IsHost)
                {
                    ...
                    m_UnityServiceErrorMessagePub.Publish(new UnityServiceErrorMessage("Host left trhe lobby", "disconnecting", UnityServiceErrorMessage.Service.Lobby));
                    await EndTracking();
            ...
            catch (LobbyServiceException e)
            {
                ...
                else if (e.Reason != LobbyExceptionReason.LobbyNotFound && !m_LocalUser.IsHost) // If lobby is not found and we are not the host, it has already been deleted. No need to publish an error here.
                {
                    PublishError(e);
                }
            }
        ...
        /// </summary>
        public async Task<(bool Success, Lobby Lobby)> TryQuickJoinAsync()
        {
        ...
        public async Task LeaveLobbyAsync(string lobbyId)
        {
            ...
            {
                // If Lobby is not found and if we are not the host, it has already beemn deleted. No need to publish the error here.
                if (e.Reason != LobbyExceptionReason.LobbyNotFound && !m_LocalUser.IsHost)
                {
                    PublishError(e);
                }
        ...
        public async Task DeleteLobbyAsync(string lobbyId)
        {
            ...
            {
                Debug.LogError("Only the host can delte a lobby.");
            }
        }
        ...
        public async Task UpdatePlayerDataAsync(Dictionary<string, PlayerDataObject> data)
        {
            ...
            catch (LobbyServiceException e)
            {
                ...
                else if (e.Reason != LobbyExceptionReason.LobbyNotFound && !m_LocalUser.IsHost) // If lobby is not found and we are not the host, it has already beemn deleted. No need to publish the error here.
                {
                    PublishError(e);
                }
            }
        ...
        /// <summary>
        /// Lobby can be provided info about Realy (or any other remote allocation) so it can add automatic disconnect handling.
        /// </summary>
        ...
        public async Task UpdateLobbyDataAsync(Dictionary<string, DataObject> data)
        {
            ...
            // we would want to lock lobbies from appearing in queries if we're in relay mode and the relay isn't fully setup yet.
        ...
        {
            m_HeartbeatTime += dt;
            if (m_HeartbeatTime > k_HearbeatPeriod)
            {
                m_HeartbeatTime -= k_HearbeatPeriod;
                try
                {
