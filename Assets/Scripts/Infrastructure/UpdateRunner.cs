...
namespace Unity.MobaRPG.Infrastructure 
{
    ...
    public class UpdateRunner : MonoBehaviour
    {
        ...
        public void Subscribe(Action<float> onUpdate, float updatePeriod)
        {
            ...
            {
                Debug.LogError("Can't Subscribe to a local function that can go oput of scope and can't be unsubscribed from");
                return;
            }

            if (onUpdate.Method.ToString().Contains("<")) // Detect anonymous functions
            {
                Debug.LogError("Can't subscribe with an anonymous function that cannot be Unusubscribed, by checking for a character that can't exist in a declared method name.");
                return;
            }
