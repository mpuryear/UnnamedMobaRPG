...
using Unity.MobaRPG.UnityServices;
using Unity.MobaRPG.UnityServices.Auth;
...
namespace Unity.MobaRPG.ApplicationLifecycle
{
    /// <summary>
    ...
    public class ApplicationController : LifetimeScope
    {
        ...
        protected override void Configure(IContainerBuilder builder)
        {
            ...
            // These message channels are essential and persist for the lifetime of the lobby and relay services
            // they are networked so that the client can subscribe to those messages that are published by the server
            ...
            builder.RegisterComponent(new NetworkedMessageChannel<CombatLogMessage>()).AsImplementedInterfaces();
            builder.RegisterComponent(new NetworkedMessageChannel<ItemLootedMessage>()).AsImplementedInterfaces();
            ...
            // LobbyServiceFacade is registered as entrypoint because it wasnts a callback after container is built to do it's initialization
        ...
        {
            // We want to quit anyways, so if anything happens while trying to loeave the lobby, log the exception then carry on
            try
            {
                m_LobbyServiceFacade.EndTracking();
            }
