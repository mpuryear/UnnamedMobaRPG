...
namespace Unity.MobaRPG.ConnectionManagement
{
    ...
    class ConnectionMethodRelay : ConnectionMethodBase
    {
        ...
        public override async Task SetupClientConnectionAsync()
        {
            ...
            Debug.Log($"Setting up Unity Relay Client for {playerId}: {m_PlayerName}");
        ...
        {
            var playerId = GetPlayerId();
            Debug.Log($"Setting up Unity Relay Host for {playerId}: {m_PlayerName}");

            SetConnectionPayload(playerId, m_PlayerName); // Need to set connection payload for host as well, as host is a client too
